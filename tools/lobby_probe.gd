...
func _client_beats(screen: Node) -> bool:
	if _stage == 0 and _t > 2.5:
		...
		_check("took the HOST's difficulty from the WELCOME packet (%d, not %d)"
			% [TIER_HARD, TIER_EASY], _tier() == TIER_HARD)
		# 2
