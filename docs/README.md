...
| **Ambience loops** — `eskinita_street.ogg`, `bayan_plaza.ogg` | *Kresiek The Furry*, via [OpenGameArt](https://opengameart.org), **CC0 1.0** — verbatim legal code and both source URLs in `assets/audio/ambience/OPENGAMEART_CC0_LICENSE.txt` | One looping background bed per map, on the Music bus (`Checklist.md` 4.1). CC0 requires no attribution; this row and that file are courtesy, and Form 03 copy-pastes from them. **The only third-party audio in the build.** |
| **All 32 sound effects** | **Ours.** Synthesised by `tools/audio/generate_sfx.py` from numpy/scipy maths | No microphone, no sample library, no download — every impact, whoosh, fanfare and UI click in the game is generated. This is why the audio disclosure on Form 03 is two CC0 rows and one "own work" row rather than a list of thirty. |
