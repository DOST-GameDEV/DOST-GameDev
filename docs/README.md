...
| **"flip flops by Tiff Eidmann"** (`aVbWCpQLno8.glb`) | ⛔ **NOT ESTABLISHED — a human must confirm before this ships** | Intended as the tsinelas mesh (checklist 7.3). That filename is the Poly Pizza convention and those models are typically **CC-BY, which REQUIRES attribution** — unlike everything else on this page. Blocking item `Checklist.md` **7.0**. If the licence cannot be confirmed, the fallback is free: the procedural tsinelas already matches the moodboard and stays. |
