...
| **4** | [🌐 NET](#lane-net) | Claude Opus 5 · high | 🟡 IN PROGRESS | 2026-07-30 · this session. **B-145 FIXED — five causes, four real peers now 27/27 · 28/28 · 28/28 · 21/21.** A third-party peer never learned anybody else's pick (invisible with two peers, which is why every run was green); a bot Prop beside a human teammate inherited nothing; solo units drew their model before the picks were written; the visual cache key ignored the prop skin; and a tsinelas's own LAKAS never rode its own throw. **R-26 net half DONE** — four-peer lobby, distinct seats under contention, mid-lobby disconnect returns the chair, and a late joiner SHUTS the START gate. **R-25 MEASURED** — teardown is 0.05 s with zero orphans, but abrupt (5.20 s) and graceful (5.40 s) quits are the SAME number, so the host must announce itself; do NOT shorten `ENET_TIMEOUT_MIN`. **R-24 DONE** — all four drop cases on three real peers, including rejoin to its own seat with the camera back. **B-144 CLOSED** (audit geometry, `PASS_WITHIN` untouched, exit 0). **NET-1(c) CLOSED.** **R-06 leg 3 + the lob constant** done. **NET-6/NET-7** answered. ⚠️ **BALANCE IS FROZEN** by 🧑 pending a mechanics overhaul — measure and report only. ⚠️ **OWED TO OTHER LANES:** `character_visual.gd` (🩴 ART) and `carriable.gd` (🥊 PHYS) were edited on 🧑's direct instruction — **flag on merge**; R-25's confirmation dialog and R-26's lobby screens are 🖥️ UX's. **STILL OPEN: R-23's loss/latency shim and the four-machine human run.** |
