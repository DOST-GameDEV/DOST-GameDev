...
- [ ] 4.22 **`tutorial.gd` now teaches a format and a mode that do not exist. This is §8.5, filed rather than edited because §8.5 forbids this lane from touching the file.** Four rows, on top of §4.10's two and §4.19's four: **(a)** any "BEST OF 5" / first-to-3 framing — the match is **first to 2 SETS**, a set being two rounds in which both teams attack exactly once (`Design.md` §7.1); **(b)** "SWAP EVERY ROUND" is no longer the whole rule — the swap is a schedule derived from `(set, round_in_set)`, and *which team attacks first alternates by set*; **(c)** the **DENTS** row and anything about the host picking a mode — Option A is deleted (`Design.md` §7.2); **(d)** the HOW YOU WIN page's two-mode structure collapses to one ruleset — attackers win on the countdown, a direct Ground Smash, or `FALL_LIMIT` 4; defenders win only on the 90 s clock. ⚠️ **§4.17 depends on this** — do not build a playable range around a deleted mode
- [ ] 4.24 **The rejoin blank screen is FIXED and MEASURED — what is left for you is the two-peer confirmation.** 🧑 2026-07-31: *"blank screen when rejoining sa ongoing na match."* **Root cause:** a rejoining peer loads a fresh `Main.tscn`, so its own `_index_to_character` starts EMPTY and the four characters arrive as `MultiplayerSpawner` deliveries — while the host's `_rpc_reclaim_character(index, peer_id)` is a reliable RPC *racing them*. When it lost that race it hit `if character == null: return` and **nothing ever retried**: authority never migrated on that peer, `_refresh_rig_ownership()` never ran, no `CameraRig` became current. B-65's token-keyed seat had already succeeded, which is why it read as "the match is gone" rather than as a seating bug. **Fix:** remember it in `_pending_reclaims`, replay it from the spawn path — the identical both-directions pattern `_rpc_sync_picks`/`_known_picks` already uses in this file. The replay is `call_deferred` because a `spawn_function` runs **before the node is in the tree** and `Camera3D.current` on an out-of-tree camera does nothing. **Measured, `tools/rejoin_probe.tscn` (new), which stages the race in one process and was run against both builds:** pre-fix **2 of 4**, failing *"a reclaim for an unknown seat is REMEMBERED"* and *"is LOOKING THROUGH it"* with **`rig_active=false camera_current=false`** — the blank screen itself; post-fix **4 of 4**, `rig_active=true camera_current=true`. ⚠️ **Note which half was never broken:** the ownership check passes in BOTH runs. Authority migration was fine; only the VIEW was lost, which is why the bug looked like a rendering failure rather than a netcode one. ⚠️ **Still owed and it is yours:** a real ENet run — join, drop, rejoin mid-match — because this probe proves the ordering contract and does not touch the wire
...
- [ ] 6.11 **This lane wrote in your block, and it is now measured with a control.** 🧑: *"jumping position bug ... can still move js stuck there"*, then *"u didnt fix stuck in jump for multiplayer"*. **Cause:** every unit shares collision layer 1, so a capsule resting on another capsule is a legal floor — `is_on_floor()` returns true in mid-air, the `if not grounded` gravity branch never runs, and the unit hovers with full walking control, which is exactly what "can still move" reports. **The first attempt was wrong and is worth knowing why:** it shed the perch with a horizontal nudge only, and a nudge cannot survive one frame here — the movement block ASSIGNS (`velocity.x = direction.x * speed_now`), so held input overwrites it, and with no input `FRICTION` 30 erases 2.5 m/s in ~0.08 s. **The real fix is `_perched_on_character`**, set from `move_and_slide()`'s own contact list and read by the gravity branch, so another unit's head simply stops counting as floor. Gravity acts on `velocity.y` and nothing in the movement block overwrites it. Side contacts are still untouched — that is the body block (`Design.md` §3.1). **Measured, `tools/perch_probe.tscn` (new), run against both builds:** pre-fix **0 of 2** with the rider RESTING at y 1.90 on a support at 0.90; post-fix **2 of 2**, already down at 1.05 one second after landing. ⚠️ **Two harness corrections are baked into that probe and both produced false PASSES first** — teleporting the rider onto the head starts the capsules overlapping so depenetration drops it either way, and asserting on the FINAL position passes on both builds because the two drift apart eventually. It drops from a gap and measures at one second. ⚠️ **`PERCH_NORMAL_MIN` 0.7 and `PERCH_SHED_SPEED` 2.5 are still first guesses**, and this does not explain all of `phys_probe`'s `*** LAUNCHED ***` verdict (`TeamBProp peak y 2.73`)
...
### 8 · 📋 `build rules` — Match format and the second ruleset *(Opus 5 · high)* — **DONE 2026-07-31**
...
- [x] 8.3 **DONE — `Design.md` §7.2 "Removed: Option A" is written, and it was written BEFORE the removal.** Records what the mode was (the dent health bar at `MAX_DENTS` 3, `clear_dent()` on the reset channel, ring-outs at `RING_OUT_LIMIT` 3, the defender timer win, the four `CAN_MESHES` damage states), why it went, and **what was kept on purpose**: `seal()`/`SEALED` survive as the state machine's shape — four call sites test that state and collapsing it into `DOWNED` would change what they mean — the kill plane still respawns but no longer scores, and the MODE row stays in `MatchSetup.tscn` for §4.18
...
**§8.2/§8.3 — Option A is deleted, and `Design.md` §7.2 was written BEFORE the removal**,
...
**THE REJOIN BLANK SCREEN WAS THE LAST OPEN BUG ON 🧑'S LIST, AND IT IS THE SAME MISTAKE
THIS FILE ALREADY MADE ONCE.** `_rpc_reclaim_character` assumed the character it names has
already arrived. On a rejoining peer it has not — that peer's `Main.tscn` is seconds old and
its four characters are still in flight from `MultiplayerSpawner` — so the RPC hit
`if character == null: return` and the reconnect was lost with no error anywhere.
`_rpc_sync_picks` hit precisely this race earlier and its own note states the rule: *"the
spawn order and this message have no guaranteed relationship, and assuming one is how the
first version of this dropped the seat that arrived a frame late."* It remembers and
replays; this now does too, via `_pending_reclaims`.

**Measured with a new probe rather than asserted, and the control is the point.**
`tools/rejoin_probe.tscn` stages the race in a single process — deliver the reclaim first,
the character second — and was run against BOTH builds. Pre-fix: **2 of 4**, with
`rig_active=false camera_current=false`, which is the blank screen reproduced rather than
described. Post-fix: **4 of 4**. ⚠️ **The ownership check passes in both runs**, and that is
worth keeping: authority migration was never broken, only the VIEW, which is exactly why the
symptom looked like a rendering fault and why the bookkeeping-side fixes of 2026-07-28 did
not touch it. ⚠️ The probe deliberately tolerates `_pending_reclaims` being absent so it runs
on the old build at all — a crash would have said "this code is different", not "this code is
broken". **It does not touch ENet**; the real join/drop/rejoin is filed on 🖥️ 4.24.
...
§7.25–7.17 filed onto existing unrun lanes. § THE RUBRIC at the top of this file records which
