...
## Sends the aim POINT and the launch ORIGIN rather than a direction — see
## _aim_point() and _throw_origin() for why each. Both are camera-derived, so
## both have to be resolved on the peer that owns the camera and travel from
## there; the host still owns whether the throw happens and how it flies.
func _request_throw(power: float) -> void:
	var target_point := _aim_point()
	var origin := _throw_origin()
	if _is_host():
		_held.host_throw(origin, target_point, power)
	else:
		_rpc_request_throw.rpc_id(1, origin, target_point, power)
...
@rpc("any_peer", "call_remote", "reliable")
func _rpc_request_throw(origin: Vector3, target_point: Vector3, power: float) -> void:
	if not _is_host() or _held == null:
		return
	_held.host_throw(origin, target_point, clampf(power, 0.0, 1.0))
